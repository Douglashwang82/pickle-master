...
export async function POST(request: Request, { params }: Params) {
  ...
  // Find the unpaid (initiated) payment transaction for this user + session
  const { data: payment } = await supabaseAdmin
    .from("payment_transactions")
    .select("id, amount_twd, status")
  ...
  // Stamp the notification time on the transaction
  await supabaseAdmin
    .from("payment_transactions")
    .update({ debt_notified_at: now, updated_at: now })
    .eq("id", payment.id);
  ...
  await supabaseAdmin.from("notifications").insert({
    ...
    payload_json: {
      ...
      message: `Reminder: please pay NT$${payment.amount_twd} for "${session.title}".`,
    },
  });
