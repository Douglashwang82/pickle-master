...
import { PublicSessionsQuerySchema } from "@/lib/validations/sessions";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import PublicSessionSearch from "@/components/sessions/PublicSessionSearch";
import SessionMapLoader from "@/components/sessions/SessionMapLoader";
...
async function fetchSessions(sp: SearchParams): Promise<{
  ...
  const parsed = PublicSessionsQuerySchema.safeParse({
    q: sp.q,
  ...
  const { q, district, price, availability, sort, view, page } = parsed.success
    ? parsed.data
    : {
        q: undefined,
        district: undefined,
  ...
  let query = supabaseAdmin
    ...
    .select(
      `
        ...
        status,
        created_at,
        clubs!inner(id, name, slug, status, district)
      `
    )
  ...
  let countQuery = supabaseAdmin
    .from("sessions")
    .select("id, clubs!inner(status, district)", { count: "exact", head: true })
    .gte("scheduled_start_at", now)
    .eq("clubs.status", "active");
  ...
  if (availability === "open") {
    ...
    countQuery = countQuery.eq("status", "full");
  } else {
    query = query.in("status", ["published", "full"]);
    countQuery = countQuery.in("status", ["published", "full"]);
  }
  ...
  if (district) {
    query = query.eq("clubs.district", district);
    countQuery = countQuery.eq("clubs.district", district);
  }

  if (price === "free") {
    query = query.eq("fee_twd", 0);
    countQuery = countQuery.eq("fee_twd", 0);
  } else if (price === "paid") {
    query = query.gt("fee_twd", 0);
    countQuery = countQuery.gt("fee_twd", 0);
  }

  if (sort === "newest") {
    query = query.order("created_at", { ascending: false });
  } else if (sort === "lowest_fee") {
    query = query.order("fee_twd", { ascending: true }).order("scheduled_start_at", { ascending: true });
  } else if (sort === "highest_fee") {
    query = query.order("fee_twd", { ascending: false }).order("scheduled_start_at", { ascending: true });
  } else {
    query = query.order("scheduled_start_at", { ascending: true });
  }

  const { data: sessionsRaw } = await query.range(offset, offset + LIMIT - 1).returns<
    Array<{
      id: string;
      title: string;
  ...
  if (sessionIds.length > 0) {
    ...
    for (const reg of regs ?? []) {
      regCountMap[reg.session_id] = (regCountMap[reg.session_id] ?? 0) + 1;
    }
  }

  const result: PublicSession[] = sessions.map((session) => {
    const club = Array.isArray(session.clubs) ? session.clubs[0] : session.clubs;
    ...
    return {
      ...
      confirmed_count: regCountMap[session.id] ?? 0,
      available_spots: session.capacity - (regCountMap[session.id] ?? 0),
      club_name: club?.name ?? "Unknown club",
      club_slug: club?.slug ?? "",
      club_district: club?.district ?? null,
    };
  });
...
export default async function PublicSessionsPage(props: {
  ...
  const { sessions, total, page, view } = await fetchSessions(searchParams);
  ...
  return (
    <div className="relative space-y-8 animate-in fade-in duration-500">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl space-y-4">
            ...
            <p className="max-w-xl text-base leading-8 text-muted-foreground md:text-lg">
              瀏覽即將舉辦的場次、查看名額與費用，先把你要加入的那一場看清楚之後再出手。
            </p>
          </div>
          ...
          <Card className="rounded-[1.5rem] border-border/70 bg-background/80 shadow-none">
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div>
                <p className="text-[0.65rem] font-black uppercase tracking-[0.2em] text-primary/70">
                  即將開打
                </p>
                <p className="text-sm font-semibold text-foreground">
                  {total > 0 ? `共有 ${total} 場公開可瀏覽的場次` : "目前沒有符合條件的場次"}
                </p>
              </div>
            </CardContent>
          </Card>
      ...
      {view === "map" && <SessionMapLoader sessions={sessions} />}
        ...
        <>
          {sessions.length > 0 ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-border/60 bg-card/90 px-4 py-20 text-center shadow-[0_24px_80px_-60px_rgba(16,42,31,0.45)]">
              ...
              <p className="mb-2 text-xl font-bold">找不到符合條件的場次。</p>
